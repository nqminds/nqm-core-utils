module.exports = (function() {
  ...
  var indexToTDX = function(indexFields, errList) {
    ...
    if (Array.isArray(indexFields)) {
      _.forEach(indexFields, (fieldSpec, idx) => {
      ...
      _.forEach(indexFields, (sortDirection, key) => {
        if (sortDirection === -1) {
  ...
  var definitionToMongoose = function(name, tdxSchema) {
    ...
    // manually below.
    //        
    log("transforming schema %s", tdxSchema.id);
    var errList = [];
    ...
    log("schema transformed from %j to %j", tdxSchema.dataSchema, mongooseSchema);
